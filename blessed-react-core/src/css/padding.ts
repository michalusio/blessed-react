...
export function splitPaddings(key: string, value: string): [string, string][] | null {
  ...
  if (sidePadding) {
    ...
    return ['top', 'right', 'bottom', 'left'].map(side => (['padding-' + side, value]));
  }
  return null;
}

export function applyPaddingStyling(classData: Record<string, string>, element: AnyElement): Record<string, string> {
  const {
  ...
  ? { top: element.style.padding, right: element.style.padding, bottom: element.style.padding, left: element.style.padding }
  : { top: 0, right: 0, bottom: 0, left: 0, ...(element.style.padding ?? {})};
