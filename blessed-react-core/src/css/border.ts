...
export function applyBorderStyling(
  ...
  const borderStyle = modeArray([
    borderTopStyle,
    borderRightStyle,
    borderBottomStyle,
    borderLeftStyle,
  ]);
  const borderWidth = modeArray([
    borderTopWidth,
    borderRightWidth,
    borderBottomWidth,
    borderLeftWidth,
  ...
  if (
    ["none", "hidden"].includes(borderRightStyle) ||
    borderRightWidth === "0"
  ) {
    borderData.right = false;
  }
  if (
    ["none", "hidden"].includes(borderBottomStyle) ||
    borderBottomWidth === "0"
  ) {
    borderData.bottom = false;
  }
...
export function splitBorders(
  ...
  if (key === "border") {
    ...
    return [
      ...parts
        .filter((part) => part.type === "color")
        .map((part) => ["border-" + part.type, part.value] as [string, string]),
      ...parts
        .filter((part) => part.type !== "color")
        .flatMap((part) =>
          ["top", "right", "bottom", "left"].map(
            (side) =>
              ["border-" + side + "-" + part.type, part.value] as [
                string,
                string
              ]
          )
  ...
  if (sideBorder) {
    ...
    const parts = value
      .split(" ")
      .map((part) => identifyBorderPart(part))
      .filter((part) => part.type !== "color");
    return parts.map((part) => [
      "border-" + side + "-" + part.type,
      part.value,
    ]);
  }
  if (["border-color", "border-style", "border-width"].includes(key)) {
    ...
    return [
      ...["top", "right", "bottom", "left"].map(
        (side) =>
          ["border-" + side + "-" + part.type, part.value] as [string, string]
      ),
    ];
  }
  return null;
