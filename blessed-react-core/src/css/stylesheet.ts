...
const classIdentifierRegex =
  /^\.-?(?:[_a-zA-Z]|[\u00A0-\uFFFF])+(?:[_a-zA-Z0-9-]|[\u00A0-\uFFFF])*$/i;

export type CSSClass = Readonly<Record<string, string>>;
export type CSSStyleSheet = Record<string, CSSClass>;

export function loadStylesheetFromFile(path: string): CSSStyleSheet {
  const nodes = CSS.parse(readFileSync(path, { encoding: "utf-8" }), {
    source: path,
  });
  const rules = nodes.stylesheet.rules.filter(
    (rule): rule is CSS.CssRuleAST => rule.type === CSS.CssTypes.rule
  );
  return Object.fromEntries(
    rules.flatMap((rule) =>
      rule.selectors
        .filter((selector) => classIdentifierRegex.test(selector))
        .map((className) => [
          className.substring(1),
          Object.fromEntries(
            rule.declarations
              .filter(
                (dec): dec is CSS.CssDeclarationAST =>
                  dec.type === CSS.CssTypes.declaration
              )
