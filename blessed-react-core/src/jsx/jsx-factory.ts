...
type GetAttributesFor<T extends JSXItem> = 
  T extends Consumer<any> | typeof Fragment
  ? null | undefined
...
type GetChildrenFor<T extends JSXItem> =
  T extends Consumer<infer X>
  ? [(value: X) => JSX.Element]
  : T extends Provider<any> | typeof Fragment
  ? [BlessedNode | undefined]
...
export function jsx(
  ...
  if (isExoticComponent(tag)) {
    switch(tag.$$type) {
      ...
      case ProviderSymbol:
        return tag({value: attributes?.value}, children[0] as BlessedNode);
    }
  }
  if (typeof tag === "function") {
    return () => {
      pushHookState(getKey(tag, attributes));
      const rendered = tag(attributes === null ? undefined : attributes, children);
      popHookState();
      return {
        _name: tag.name,
        _children: flatChildren,
...
function splitAttributes<T extends keyof JSX.IntrinsicElements>(attributes: JSX.IntrinsicElements[T]): [Exclude<JSX.IntrinsicElements[T], JSX.ElementEventProperties<T>>, JSX.ElementEventProperties<T>] {
...
function isExoticComponent(tag: any): tag is typeof Fragment | Consumer<unknown> | Provider<unknown> {
