...
declare global {
  namespace JSX {
    type NodeScreenEventType = "focus" | "blur" | "click";
    ...
    type onParameters<T extends keyof elements> = Parameters<
      ReturnType<elements[T]>["on"]
    >;

    type ElementEventProperties<T extends keyof elements> = {
      [X in AddPrefix<"on", onParameters<T>[0]>]?: (
        listener: onParameters<T>[1]
      ) => void;
    } & {
      [X in AddPrefix<"on", NodeMouseEventType>]?: (
        listener: (arg: IMouseEventArg) => void
      ) => void;
    } & {
      [X in AddPrefix<"on", NodeScreenEventType>]?: (
        listener: (arg: Screen) => void
      ) => void;
    } & {
      [X in AddPrefix<"on", NodeGenericEventType>]?: (
        listener: () => void
      ) => void;
    };
    ...
    type ElementSpecialProperties<K extends keyof elements> = {
      key?: string | number;
      ref?: MutableRef<ReturnType<elements[K]>>;
      className?: CSSClass;
    };
    ...
    type IntrinsicElements = {
      [K in keyof elements]: Parameters<elements[K]>[0] &
        ElementSpecialProperties<K> &
        ElementEventProperties<K>;
    };

    type Tag = keyof IntrinsicElements;

    type Component<P extends {} = {}> = {
      (properties: P, children?: ItemOrArray<Element>[]): Element;
    };
  }
}
